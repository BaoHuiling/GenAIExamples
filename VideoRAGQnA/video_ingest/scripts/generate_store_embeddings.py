...
def init_vectordb(db: str):
    api_url = f"http://{host}:{port}/visual_rag_retriever/init_db"
    results = post_data(api_url, {"selected_db": db})
    return results
...
def store_into_vectordb(metadata_file_path, db):
    ...
    api_url = f"http://{host}:{port}/visual_rag_retriever/upload_images"
    ...
    for _, (video, data) in enumerate(GMetadata.items()):
        ...
        results = post_file(api_url, image_files, metadatas)
...
def retrieval_testing():
    ...
    api_url = f"http://{host}:{port}/visual_rag_retriever/query"
